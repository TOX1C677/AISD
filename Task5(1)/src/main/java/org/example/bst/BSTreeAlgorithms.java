...
public class BSTreeAlgorithms {
    ...
    /*
     ...
     public static <T extends Comparable<? super T>> tree.BinaryTree.TreeNode<T> getNode(tree.BinaryTree.TreeNode<T> node, T value) {
        while (node != null) {
            int cmp = node.getValue().compareTo(value);
            if (cmp == 0) {
                break;
    ...
    public static <T extends Comparable<? super T>> tree.BinaryTree.TreeNode<T> getMinNode(tree.BinaryTree.TreeNode<T> node) {
        if (node == null) {
            return null;
        }
